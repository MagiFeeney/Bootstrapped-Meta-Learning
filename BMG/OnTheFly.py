...
class OnTheFly():
    ...
    def matching_function(self, Kth_model, TB_model, states, Kth_state_dict=None, flag=True):
        if flag:
            TorchOpt.recover_state_dict(Kth_model, Kth_state_dict)

        dist_K = [Kth_model(states[i])[0] for i in range(self.steps_per_rollout)] # get Kth rollout distribution \pi_\theta{K} before being recovered
        with torch.no_grad():
            dist_T = [TB_model(states[i])[0] for i in range(self.steps_per_rollout)]

        # Calculate KL divergence loss
        KL_loss = sum([kl_divergence(dist_T[i], dist_K[i]) for i in range(self.steps_per_rollout)])
