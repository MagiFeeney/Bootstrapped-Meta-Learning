...
def main():
    ...
    outer_iters = args.max_steps // steps_per_rollout // (args.K + args.L)
    ...
    for i in range(args.num_seeds):
        ...
        if args.metaopt == "TorchOpt":
            ...
            cumulative_rewards, rew_step, entropy_rate = trainer.train(Kth_model)
        elif args.metaopt == "MetaOptim":
            bootstrap_model = deepcopy(inner_model)
            bootstrap_optimizer = optim.SGD(bootstrap_model.parameters(), lr=args.lr)
            cumulative_rewards, rew_step, entropy_rate = trainer.train(bootstrap_model, bootstrap_optimizer, args.metaopt)
    ...
    if shade_flag:
        ...
        s2_std = b.std(axis=0)
        s3_std = c.std(axis=0)
        plot(outer_iters * steps_per_rollout * (args.K + args.L), s1, s2, s3, s1_std, s2_std, s3_std, shade_flag=shade_flag)
    else:
        plot(outer_iters * steps_per_rollout * (args.K + args.L), s1, s2, s3)
