...
class Trainer():
    ...
    def train(self, flag="TorchOpt"):
        if flag == "TorchOpt":
            cumulative_rewards, rew_step, entropy_rate = self.MGRLTorchOpt()
        elif flag == "MetaOptim":
            cumulative_rewards, rew_step, entropy_rate = self.MGRLMetaOptim()

        return cumulative_rewards, rew_step, entropy_rate

    def MGRLTorchOpt(self):
        for outer_iter in tqdm(range(self.outer_iters)):    
            meta_loss = 0
            for inner_iter in range(self.T):
                loss, actor_loss, entropy = self.controller.rollout(self.inner_model, self.outer_model)
                self.inner_optimizer.step(loss)
                meta_loss = meta_loss + ((actor_loss + self.epsilon_meta * entropy) - meta_loss) / (inner_iter + 1)
